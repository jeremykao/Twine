...
window.fbAsyncInit = function() {
 ...
 $("#search-bar").bind("change", function(){
    ...
    queryStr = "SELECT username, name, pic_small, current_location.state, current_location.latitude, current_location.longitude from user where uid IN (SELECT uid FROM page_fan WHERE page_id IN (SELECT page_id FROM page WHERE name='"+ search_term+"' ) AND uid IN (SELECT uid2 FROM friend WHERE uid1=me() ))";
  });
